...
class ImagePathProcessor:
    ...
    def get_source_code(self):
        ...
        if source_code is None:
            error_info = f' INVALID SOURCE ATTRIBUTE: "{self.source}" '
...
class ImageProcessor:
    ...
    def get_image(self):
        return self.convert_content_to_image(image_content=self.image_content, mode=self.mode)
    ...
    def export(cls, image_content, mode, path, filename):
        ...
        # 不需先轉 base64
        image = cls.convert_content_to_image(image_content=image_content, mode=mode)
